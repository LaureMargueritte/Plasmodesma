...
#4. Main code
def process_1D(xarg):
    "Performs all processing of exp, and produces the spectrum (with and without peaks) and the list"
    exp, resdir = xarg
...
#    dd.display_peaks(color="g")
    ...
    if name.find('cosy') != -1 or name.find('dipsi') != -1:
        dd.bucket2d(file=bkout, zoom=( (0.1, 9.5) , (0.1, 9.5)), bsize=(0.03, 0.03) )
    elif name.find('hsqc') != -1 or name.find('hmbc') != -1:
        dd.bucket2d(file=bkout, zoom=( (-13,134) , (0.1, 9.5)), bsize=(1.0, 0.03) )
        ...
        dd.bucket2d(file=bkout, zoom=( (ldmin, ldmax) , (0.1, 9.5)), bsize=(0.1, 0.03) ) #original parameters
    else:
...
def main(DIREC, Nproc):
    ...
    if Nproc > 1:
        ...
        POOL = mp.Pool(Nproc)
        # this forces to use MKL only one thread (faster !)
        try:
            mkl_rt = ctypes.CDLL('libmkl_rt.dll')
